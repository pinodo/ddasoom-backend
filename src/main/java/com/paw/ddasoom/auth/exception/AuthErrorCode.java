...
public enum AuthErrorCode implements ErrorCode{
  ...
  // AUTH_006(이메일 단위 쿨다운)과 별도 코드인 이유: 프론트가 AUTH_006을 "1분에 한 번" 문구로
  // 분기 중이라, 성격이 다른 IP 단위 제한(시간당)을 같은 코드에 실으면 안내가 틀려짐
  AUTH_CODE_IP_LIMIT(HttpStatus.TOO_MANY_REQUESTS, "AUTH_007",
          "인증 메일 요청이 너무 많습니다. 1시간 후 다시 시도해 주세요."),
  ...
  ----------------------------- */
   ...
   /**---------------------------- 
     회원 제재(숨김) 관련 에러코드 
  ----------------------------- */
   // 신고 제재로 숨김(HIDDEN) 처리된 회원의 로그인 차단 전용 코드.
   // 탈퇴(AUTH_109)와 구분하는 이유: 프론트가 배너 문구를 다르게 안내해야 하고(제재 vs 탈퇴),
   // 두 상태는 해소 경로도 다르다(탈퇴=복구 문의 / 숨김=제재 관련 문의).
   BLOCKED_MEMBER(HttpStatus.FORBIDDEN, "AUTH_110",
          "이용이 제한된 계정입니다. 자세한 사항은 1:1 문의를 이용해 주세요.");
